...
// CRC table
static const uint32_t CRC_Table32[256] =
{
...
static const uint8_t CRC_ToLower[256] =
{
...
namespace LibSWBF2
{
	CRCChecksum CRC::CalcLowerCRC(const char* str)
	{
		...
		{
			crc = (crc << 8) ^ CRC_Table32[(crc >> 24) ^ CRC_ToLower[*p]];
		}
		return ~crc;
	}
	...
	{
		static std::unordered_map<FNVHash, std::string> singleton;
		...
		for (std::string line; std::getline(csv, line); )
		{
			...
			singleton.emplace(Hash(line.c_str()), line);
		}
#endif
	...
	{
#ifdef LOOKUP_CSV_PATH
		const auto it = LookupTable().find(hash);
